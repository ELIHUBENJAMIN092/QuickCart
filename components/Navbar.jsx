  ...
  return (
    <nav className="flex items-center justify-between px-6 md:px-16 lg:px-32 py-3 border-b border-gray-300 text-gray-700">
      ...
      <div className="flex items-center gap-4 lg:gap-8 max-md:hidden">
        ...
        {isSeller && <button onClick={() => router.push('/seller')} className="text-xs border px-4 py-1.5 rounded-full">Panel de Administrador</button>}
          ...
          user
            ? <>
              <UserButton>
                ...
                <UserButton.MenuItems>
                  <UserButton.Action label="Mis Ordenes" labelIcon={<BagIcon />} onClick={() => router.push('/my-orders')} />
                </UserButton.MenuItems>
              </UserButton>
            </>
        ...
        {isSeller && <button onClick={() => router.push('/seller')} className="text-xs border px-4 py-1.5 rounded-full">Panel de Administrador</button>}
        {
          user
            ? <>
              <UserButton>
              <UserButton.MenuItems>
                  <UserButton.Action label="Casa" labelIcon={<HomeIcon />} onClick={() => router.push('/')} />
                </UserButton.MenuItems>
                <UserButton.MenuItems>
